...
(define y1 (cons "feature1" (lambda (x) (list-ref x 0))))
; returns the value of feature 1 for a given test sample
(define y2 (cons "feature2" (lambda (x) (list-ref x 1))))
(define y3 (cons "feature3" (lambda (x) (list-ref x 2))))
(define y4>62 (cons "feature4>62"
                             (lambda (x) (if (> (list-ref x 3) 62) 1 0))))
...
(define pclass (cons "pclass" (lambda (x) (list-ref x 0))))
; returns the value of pclass for a given test sample
(define sex (cons "sex" (lambda (x) (list-ref x 1))))
(define age>25 (cons "age>25" (lambda (x) (if (> (list-ref x 2) 25) 1 0))))
(define sibsp (cons "sibsp" (lambda (x) (list-ref x 3))))
(define parch (cons "parch" (lambda (x) (list-ref x 4))))
(define fare>50 (cons "fare>50" (lambda (x) (if (> (list-ref x 5) 50) 1 0))))
...
(define cshape (cons "cshape" (lambda (x) (list-ref x 0))))
(define csurf (cons "csurf" (lambda (x) (list-ref x 1))))
(define bruise (cons "bruise" (lambda (x) (list-ref x 2))))
